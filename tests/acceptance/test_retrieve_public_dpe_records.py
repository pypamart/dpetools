...
def ademe_api_reachable(dpe_api_client: DPEApiClient, url: str):
    assert url == Container.API_DATA_URL
...
@then("the response should contain at least 1 DPE record")
...
@then(parsers.cfparse('each DPE record should have a non-empty "{field}" field'))
